    ...
    public class UrlRepository(AppDbContext _context) : RepositoryBase<URL>(_context), IUrlRepository
    {
        ...
        private IQueryable<URL> FetchWithParameter(UrlParameters urlParam, CancellationToken ct, bool asNoTracking = false)
             =>  FindAll(asNoTracking)
                     ...
                     .Sort(urlParam.OrderBy! ?? "createdAt desc")
        ...
        public async Task<(IEnumerable<URL> Urls, int Count)> FetchAllOwnedUrlsAsync(UrlParameters urlParam, int userId, CancellationToken ct, bool asNoTracking = false)
        {
            List<URL> urls = await FetchWithParameter(urlParam, ct, asNoTracking).OwnedBy(userId).ToListAsync();
            int count = await FindByConditon(u => u.CreatedBy == userId).CountAsync();
            return (urls, count);
