...
namespace UrlShortener.Core.Services.Implementations
{
    ...
    public class UrlService : IUrlService
    {
        ...
        {
            if (dto.ShortCode is not null && user.Plan.HasCustomSlugs ) return dto.ToUrl(user);
            var shortCodeResult = await _sm.ShortCodeService.Generate();

            if(shortCodeResult.IsFailure) return shortCodeResult.Error; 

            dto.ShortCode = shortCodeResult.Value;
            return dto.ToUrl(user);
        }
        ...
        public async Task<Result<(IEnumerable<URL> Urls, int Count)>> RetrieveAllOwnedUrlsAsync(Result<UrlParameters> UrlParam, int userId, CancellationToken ct = default) 
                    => await UrlParam
        ...
        public async Task<Result<PagedList<FullUrlDTO>>> RetrieveAllUrlsWithPagingAsync(UrlParameters UrlParam, CancellationToken ct = default)
            => await RetrieveAllUrlsAsync(UrlParam, ct).Map(urls => PagedList<FullUrlDTO>.ToPagedList([..urls.Select(u => u.ToFullDTO())], 0, UrlParam.PageNumber, UrlParam.PageSize));
        public async Task<Result<PagedList<FullUrlDTO>>> RetrieveAllOwnedUrlsWithPagingAsync(UrlParameters UrlParam, CancellationToken ct = default)
            => await RetrieveAllOwnedUrlsAsync(UrlParam, _sm.User.Id, ct).Map(pair => PagedList<FullUrlDTO>.ToPagedList([.. pair.Urls.Select(u => u.ToFullDTO())], pair.Count, UrlParam.PageNumber, UrlParam.PageSize));
